  ...
  return (
    <div className={`rounded-2xl border border-sand-line bg-cream-50 p-5 shadow-sm ring-1 sm:p-6 ${style.ring}`}>
        ...
        <Zeile nr={2} label="MRG-Anwendungsbereich">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <span className="text-xs text-ink-faint">
              Kündigungsschutz: <strong className="text-ink-soft">{ergebnis.kuendigungsschutz ? 'ja' : 'nein'}</strong> · Preisschutz:{' '}
              <strong className="text-ink-soft">{ergebnis.preisschutz ? 'ja' : 'nein'}</strong>
            </span>
          </div>
        </Zeile>
      ...
      <div className="mt-5 space-y-3 rounded-xl bg-cream-100 p-4 text-sm">
        <div>
          <p className="mb-1 font-semibold text-ink">Begründung</p>
        ...
        <div>
          <p className="mb-1 font-semibold text-ink">Rechtsgrundlage(n)</p>
          <p className="text-ink-soft">{ergebnis.rechtsgrundlagen.join(' · ')}</p>
        </div>
        {ergebnis.lageHinweis && (
          <div>
            <p className="mb-1 font-semibold text-sage-700">Lage</p>
            <p className="text-ink-soft">{ergebnis.lageHinweis}</p>
          </div>
        )}
